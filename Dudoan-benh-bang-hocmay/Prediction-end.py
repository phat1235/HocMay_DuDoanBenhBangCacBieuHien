...
def create_symptom_data(symptoms_dict):
    """tạo cái Dataframe để cho vào mô hình chạy."""
    df = pd.DataFrame(columns=list(symptoms_dict.keys()))
    df.loc[0] = np.array(list(symptoms_dict.values()))
    return df

def predict_disease(df_test):
    """Tải mô hình mà mình đã train ok."""
    clf = load("./model/random_forest.joblib")
    result = clf.predict(df_test)
    return result

if __name__ == '__main__':
    # Đặt giá trị là 1 nếu có biểu hiện bệnh gì đó
    symptoms = {
        'itching': 1, 
        'skin_rash': 1, 
    ...
    df_test = create_symptom_data(symptoms)

    # Dự đoán bệnh
    prediction = predict_disease(df_test)
    print(f"Dự đoán bệnh: {prediction}")
